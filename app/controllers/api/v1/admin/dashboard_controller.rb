class Api::V1::Admin::DashboardController < Api::V1::AdminController
  ...
  def statistics_info
    ...
    total_paid_users = -1
    total_generated_user = User.joins(:name_based_generated_logs).distinct.count
    total_images = "#{ReplicatedCall.all.size}, #{NameBasedGeneratedLog.all.size}"
    total_paid_dollar = -1
    # total_paid_credits = 0
    # total_cost_dollar = -1
    total_cost_credits = ReplicatedCall.count() #where("name_based_generated_logs.data->>'status' = ?", 'succeeded').sum(:cost_credits)
    # total_left_dollar = -1
    # total_left_credits = total_paid_credits - total_cost_credits
    #
    today_newly_users = User.where("DATE(created_at) = ?", Date.today).count
    # today_paid_users = -1
    today_generated_users = User.joins(:name_based_generated_logs)
                                .where(name_based_generated_logs: { created_at: Time.current.beginning_of_day..Time.current.end_of_day })
                                .distinct
                                .count
    today_images = "#{ReplicatedCall.where(created_at: Time.current.beginning_of_day..Time.current.end_of_day).count}, #{NameBasedGeneratedLog.where(created_at: Time.current.beginning_of_day..Time.current.end_of_day).count}"
    # today_paid_dollar = -1
    # today_paid_credits = -1
    # today_cost_dollar = -1
    today_cost_credits = ReplicatedCall
                           .where(created_at: Time.current.beginning_of_day..Time.current.end_of_day)
                           # .where("name_based_generated_logs.data->>'status' = ?", 'succeeded')
                           .count()
                           # .sum(:cost_credits)
    ...
    # # User.joins(:name_based_generated_logs)
    # #                          .where("name_based_generated_logs.data->>'status' = ?", 'succeeded')
    # #                          .group('users.id')
    # #                          .having('COUNT(*) >= ?', 0)
    ...
    # user_top_cost_credits = User.joins(:name_based_generated_logs)
    #                             .group("users.id")
    ...
    # user_top_generated_images = User.joins(:name_based_generated_logs)
    #                                 .group("users.id")
